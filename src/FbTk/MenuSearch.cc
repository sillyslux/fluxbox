...
size_t search_str_textstart(const std::string &text,
...
size_t (*search_str)(const std::string &,
                     const std::string &) = search_str_textstart;
...
size_t MenuSearch::size() const { return pattern.size(); }

void MenuSearch::clear() { pattern.clear(); }

void MenuSearch::add(char c) { pattern.push_back(std::tolower(c)); }
...
bool MenuSearch::would_match(const std::string &the_pattern) {
  ...
  for (i = 0; i < l; i++) {
    if (!m_items[i]->isEnabled())
      continue;
    if (search_str(m_items[i]->iTypeString(), the_pattern) !=
        std::string::npos) {
      return true;
    }
  }
