...
public:
    Ev()
        : m_window(0, 0, 0, 512, 512, ExposureMask | ButtonPressMask | ButtonReleaseMask | ButtonMotionMask | SubstructureRedirectMask)
        , m_drag(m_window, 0, 0, 32, 32,
    ...
    void buttonReleaseEvent(XButtonEvent& event) { drag = false; }
    ...
    void motionNotifyEvent(XMotionEvent& event)
    {
        ...
        m_drag.move(event.x - grab_x, event.y - grab_y);
    }
    int grab_x, grab_y;
    bool drag;
