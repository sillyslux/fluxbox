...
int test_insideBorder() {
  ...
  for (unsigned int i = 0; i < sizeof(tests) / sizeof(_t); ++i) {
    ...
    printf(
        "  %u: is (%02d|%02d) inside [%d %d]-[%d %d] with border %d: %s, %s\n",
        i, t.x, t.y, t.rect.x(), t.rect.y(), t.rect.x() + (int)t.rect.width(),
        t.rect.y() + (int)t.rect.height(), t.bw, result ? "yes" : "no",
        result == t.truth ? "ok" : "failed");
  }
  ...
  struct _test {
    bool operator()(const Rect &a, const Rect &b, int truth, unsigned int i) {
      ...
      printf("  %u: [%2d %2d]-[%2d %2d] %s [%2d %2d]-[%2d %2d]: %s\n", i, a.x(),
             a.y(), a.x() + (int)a.width(), a.y() + (int)a.height(),
             result ? "overlaps" : "does not overlap", b.x(), b.y(),
             b.x() + (int)b.width(), b.y() + (int)b.height(),
