...
public:
  Application(int box_size, int num)
      : FbWindow(0, 0, 0, 640, ((5 + box_size) * num) / 9 + 5,
                 ExposureMask | KeyPressMask),
        m_box_size(box_size), m_num(num), m_font("fixed"),
        m_imgctrl(screenNumber()),
        m_background(*this, width(), height(), depth()), m_gc(m_background) {
  ...
  void exposeEvent(XExposeEvent &ev) { clear(); }

private:
  void renderPixmap(const Texture &text, int x, int y) {
    Pixmap pm = m_imgctrl.renderImage(2 * m_box_size, m_box_size, text);

    m_background.copyArea(pm, m_gc.gc(), 0, 0, x, y, 2 * m_box_size,
                          m_box_size);
  ...
  void renderPixmaps() {
    ...
    m_background.fillRectangle(m_gc.gc(), 0, 0, width(), height());
    // for text color
    ...
    for (int i = 0; i < m_num; ++i) {
      sprintf(value, "%d", i);
      text.reset(new ThemeItem<Texture>(tm, string("texture") + value,
      ...
      if (next_x + 2 * m_box_size > width()) {
        m_font.drawText(m_background, screenNumber(), m_gc.gc(), value,
                        strlen(value), next_x, next_y + m_box_size / 2);
        next_x = 5;
        next_y += step_size;
      }
    }
